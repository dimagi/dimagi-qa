...
def test_case_01_access_to_non_dr_caselist(driver, settings):
...
@pytest.mark.skip (reason= "An unexpected error occurred. Please report an issue if you continue to see this message.")
def test_case_02_load_external_linked_domain_case_into_caselist_search_first(driver, settings):
    webapps = WebApps(driver, settings)
    casesearch = CaseSearchWorkflows(driver)
...
@pytest.mark.skip (reason= "An unexpected error occurred. Please report an issue if you continue to see this message.")
def test_case_03_smart_link_search_first_linked_domain_case(driver, settings):
    webapps = WebApps(driver, settings)
    casesearch = CaseSearchWorkflows(driver)
...
def test_case_04_unrelated_case_property(driver, settings):
    webapps = WebApps(driver, settings)
    casesearch = CaseSearchWorkflows(driver)
    webapps.open_domain(domain_name=CaseSearchUserInput.casesearch, current_url=driver.current_url)
    webapps.login_as(CaseSearchUserInput.user_1)
...
@pytest.mark.skip (reason= "An unexpected error occurred. Please report an issue if you continue to see this message.")
def test_case_05_load_external_linked_domain_case_into_caselist_search_first_parent_child(driver, settings):
    webapps = WebApps(driver, settings)
    casesearch = CaseSearchWorkflows(driver)
    webapps.login_as(CaseSearchUserInput.user_2)
    webapps.open_app(CaseSearchUserInput.case_search_app_name)
    ...
    assert "casesearch" in domain_url, "casesearch not present in url"
    print("casesearch present in url")
    webapps.open_form(CaseSearchUserInput.search_duplicate_form)
    webapps.select_case_and_continue(casename)
    webapps.submit_the_form()